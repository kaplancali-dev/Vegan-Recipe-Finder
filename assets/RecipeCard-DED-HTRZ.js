import{n as r,e as l,i as A,j as E}from"./index-BII89Y8t.js";import{I as w,G as P,S as R}from"./matching-DpFccogD.js";function U(t,e){const i=r(t);for(const a of e){const n=r(a),o=w[n];if(o&&o.some(s=>r(s)===i))return a}const c=w[i];if(c){for(const a of c)if(e.some(n=>r(n)===r(a)))return a}return null}function Q(t){return t>=80?"hi":t>=40?"md":"lo"}const $=new Map;for(const[t,e]of Object.entries(P))$.set(r(t),e);function W(t){const e=r(t),i=$.get(e);if(i)return i;let c=null,a=0;for(const[n,o]of $)e.includes(n)&&n.length>3&&n.length>a&&(c=o,a=n.length);return c}const b=new Map;for(const[t,e]of Object.entries(R))b.set(r(t),e);function Y(t){const e=r(t),i=b.get(e);if(i)return i;for(const[c,a]of b)if(e.includes(c)&&c.length>3)return a;return null}function M(t,e){const i=A(E(t)),c=W(t),a=Y(t),n=c?`<span class="gf-swap">GF: ${l(c)}</span>`:"",o=a?`<span class="sf-swap">Swap: ${l(a)} to significantly reduce sugar calories/carbs</span>`:"";return`<span class="${e}${c?" c-gluten":a?" c-sugar":""}">${l(i)}${n}${o}</span>`}function z(t,e={}){var S;const{showMatch:i=!0,isFavorite:c=!1,isOnMakeList:a=!1,cookedDates:n=[],userIngs:o=[]}=e,s=t,h=Q(s.pct),L=s.img?" has-hero":"",C=s.img?`<div class="hero-wrap"><img loading="lazy" decoding="async" src="${l(s.img)}" alt="${l(s.title)}"></div>`:"",T=i&&s.pct!==void 0?`<span class="match-pill ${h}">${s.pct}%</span>`:"",j=s.time?`<span>⏱ ${s.time} min</span>`:"",x=s.servings?`<span>👤 ${s.servings} servings</span>`:"",p=s.nut||{},F=s.nut?`
    <div class="nut-row">
