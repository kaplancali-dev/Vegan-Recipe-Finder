import{b as $,$ as x,d as k,e as n,g as U,f as P,h as j,i as V,j as Y,k as z}from"./index-CCb57u9F.js";import{f as J}from"./matching-GxHxreHO.js";import{g as G,s as K,f as W}from"./RecipeCard-BlUVavuW.js";import{g as X,h as Z,t as tt,s as st,a as et,o as nt}from"./RecipeDetail-C6jHV2oX.js";import"./supabase-DaN4is7k.js";import"./InstallCardDesktop-DVxK9suI.js";function at(i){let t=0;for(let e=0;e<i.length;e++)t=(t<<5)-t+i.charCodeAt(e)|0;return Math.abs(t)}const it=new Set(["Dessert","Baking","Snack","Breakfast","Smoothie"]),ot=new Set(["Lazy Cat Kitchen","Rainbow Plant Life","Pick Up Limes","Vegan Richa","Minimalist Baker","Loving It Vegan"]);function rt(i){const t=i.filter(a=>{var f;return a.img&&a.nut&&a.nut.pro>=20&&a.ing&&a.ing.length>=10&&a.ing.length<=15&&a.time&&a.time<=90&&!((f=a.cats)!=null&&f.some(S=>it.has(S)))});if(!t.length)return null;const e=t.filter(a=>a.site&&ot.has(a.site)),p=e.length>=30?e:t,m=new Date,w=`harvest-${m.getFullYear()}-${m.getMonth()+1}-${m.getDate()}`,I=at(w)%p.length;return p[I]}let y=null;function ht(i){if(y=rt(i),!y)return;E();let t=0;const e=()=>{cancelAnimationFrame(t),t=requestAnimationFrame(E)};$("ingredients",e),$("staples",e),$("favorites",e),$("makelist",e),$("cookHistory",e)}function E(){const i=x("#rotdSpotlight");if(!i||!y)return;const t=y,e=k("ingredients"),p=k("staples"),m=new Set(k("favorites")),w=k("makelist"),I=k("cookHistory")||[],f=J({recipes:[t],ingredients:e,staples:p,selectedCats:[],allergies:new Set})[0]||{...t,pct:0,haveNames:[],needNames:t.ing||[]},S=m.has(t.id),T=w.includes(t.id),M=I.filter(s=>s.id===t.id),l=M.length?M[M.length-1]:null,b=t.nut||{},O=e.length>0||p.length>0?`<span class="rotd-match">${f.pct}% match</span>`:"",L=f.haveNames||[],v=f.needNames||[],B=[...e,...p];function F(s,o){const r=n(Y(z(s))),h=G(s),g=K(s);let c="";if(h&&(c+=` <em class="rotd-hint-gf">(GF: ${n(h)})</em>`),g&&(c+=` <em class="rotd-hint-sf">(Lower-carb: ${n(g)})</em>`),o&&B.length){const d=W(s,B);d&&(c+=` <em class="rotd-hint-sub">(💡 try ${n(d)})</em>`)}return c?`<span class="${h?"rotd-ing-gf":g?"rotd-ing-sf":""}">${r}${c}</span>`:r}const D=L.length?`<span class="rotd-have"><strong>You have:</strong> ${L.map(s=>F(s,!1)).join(", ")}</span>`:"",N=v.length?`<span class="rotd-need"><strong>You need:</strong> ${v.map(s=>F(s,!0)).join(", ")}</span>`:"",C=(t.cats||[]).map(s=>`<span class="rotd-cat">${n(s)}</span>`).join(""),Q=[...t.ing||[]],H=new Set,R=[];for(const s of Q){const o=X(s);if(o&&o.benefits)for(const r of o.benefits)H.has(r)||(H.add(r),R.push(r))}const q=R.length?R.slice(0,5).map(s=>`<span class="rotd-benefit">✦ ${n(s)}</span>`).join(""):"";let A="☐ I Made This";if(l){const s=new Date(typeof l=="string"?l:l.date).toLocaleDateString(void 0,{month:"numeric",day:"numeric"}),o=typeof l=="object"&&l.rating?" "+"★".repeat(l.rating):"";A=`✅ Made ${s}${o}`}i.innerHTML=`
