import{g as E,n as h,f as m,i as L,h as f,a as H,$ as S,d as $,e as C,l as G}from"./index-DsxOKTSs.js";import{f as D,G as F}from"./matching-BnBvY7uw.js";import{o as Y}from"./RecipeDetail-D3rLNIMu.js";import"./supabase-DaN4is7k.js";import"./InstallCardDesktop-DVxK9suI.js";let T=[];function y(){const t=$("shopChecked");return new Set(Array.isArray(t)?t:[])}function j(t){m("shopChecked",[...t])}function ee(t){T=t||[],q(),x(),H(["shopList","shopChecked","shopRecipes","ingredients","staples"],"shopping",x)}function q(){const t=S("#clearShopBtn");t&&t.addEventListener("click",()=>{const o=$("shopRecipes"),c=$("shopList");!o.length&&!c.length||(m("shopRecipes",[]),m("shopList",[]),m("shopChecked",[]),L(),f("Clean slate — list wiped"))});const s=S("#shareShopBtn");s&&s.addEventListener("click",()=>{z()})}function z(){const{recipeCards:t,manualItems:s}=_();if(!t.length&&!s.length){f("Nothing to share yet — queue some recipes first");return}const o=y(),c="HARVEST Shopping List",a=[];if(t.forEach(({title:p,missing:r})=>{const d=r.filter(i=>!o.has(h(i))),e=r.filter(i=>o.has(h(i)));let l=`🍽 ${p}`;d.length&&(l+=`
`+d.map(i=>`  • ${i}`).join(`
`)),e.length&&(l+=`
`+e.map(i=>`  ✓ ${i}`).join(`
