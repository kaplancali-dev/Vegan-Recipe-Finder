import{g as I,n as l,c as m,f as L,d,s as N,$ as y,b as v,e as b,j as _}from"./index-BsARknOr.js";import{f as A,G as P}from"./matching-DfvbBxPl.js";import{o as H}from"./RecipeDetail-Dwy_yk8U.js";import"./supabase-DaN4is7k.js";let T=[];function k(){const t=v("shopChecked");return new Set(Array.isArray(t)?t:[])}function M(t){m("shopChecked",[...t])}function K(t){T=t||[],G(),E(),N(["shopList","shopChecked","shopRecipes","ingredients","staples"],"shopping",E)}function G(){const t=y("#clearShopBtn");t&&t.addEventListener("click",()=>{const i=v("shopRecipes"),r=v("shopList");!i.length&&!r.length||(m("shopRecipes",[]),m("shopList",[]),m("shopChecked",[]),L(),d("Clean slate — list wiped"))});const s=y("#shareShopBtn");s&&s.addEventListener("click",()=>{D()})}function D(){const{recipeCards:t,manualItems:s}=x();if(!t.length&&!s.length){d("Nothing to share yet — queue some recipes first");return}const i=k(),r="HARVEST Shopping List",a=[];if(t.forEach(({title:h,missing:o})=>{const f=o.filter(c=>!i.has(l(c))),e=o.filter(c=>i.has(l(c)));let p=`🍽 ${h}`;f.length&&(p+=`
`+f.map(c=>`  • ${c}`).join(`
`)),e.length&&(p+=`
`+e.map(c=>`  ✓ ${c}`).join(`
