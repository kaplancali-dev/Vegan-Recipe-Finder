import{g as I,n as h,c as m,f as S,d,s as H,$ as L,b as $,e as C,j as G}from"./index-D9OSTMFa.js";import{f as D,G as F}from"./matching-CphHQH1p.js";import{o as Y}from"./RecipeDetail-CUKgaQWa.js";import"./supabase-DaN4is7k.js";let T=[];function y(){const t=$("shopChecked");return new Set(Array.isArray(t)?t:[])}function j(t){m("shopChecked",[...t])}function Z(t){T=t||[],q(),N(),H(["shopList","shopChecked","shopRecipes","ingredients","staples"],"shopping",N)}function q(){const t=L("#clearShopBtn");t&&t.addEventListener("click",()=>{const i=$("shopRecipes"),r=$("shopList");!i.length&&!r.length||(m("shopRecipes",[]),m("shopList",[]),m("shopChecked",[]),S(),d("Clean slate — list wiped"))});const s=L("#shareShopBtn");s&&s.addEventListener("click",()=>{z()})}function z(){const{recipeCards:t,manualItems:s}=_();if(!t.length&&!s.length){d("Nothing to share yet — queue some recipes first");return}const i=y(),r="HARVEST Shopping List",a=[];if(t.forEach(({title:l,missing:o})=>{const f=o.filter(c=>!i.has(h(c))),e=o.filter(c=>i.has(h(c)));let p=`🍽 ${l}`;f.length&&(p+=`
`+f.map(c=>`  • ${c}`).join(`
`)),e.length&&(p+=`
`+e.map(c=>`  ✓ ${c}`).join(`
