import{s as h,v as b,c as g}from"./index-CTOA2ZtO.js";import"./supabase-DaN4is7k.js";const y=3e4;let n=null,_=0;function v(){const e=document.createElement("div");return e.id="signInModal",e.className="sin-modal",e.hidden=!0,e.setAttribute("role","dialog"),e.setAttribute("aria-modal","true"),e.setAttribute("aria-labelledby","sinTitle"),e.innerHTML=`
  ...
  `,document.body.appendChild(e),S(e),e}function m(e){n&&(n.querySelectorAll("[data-sin-step]").forEach(i=>{i.hidden=Number(i.dataset.sinStep)!==e}),setTimeout(()=>{var i,s;e===1&&((i=n.querySelector("#sinEmail"))==null||i.focus()),e===2&&((s=n.querySelector("#sinCode"))==null||s.focus())},50))}function f(){n&&(n.hidden=!0),document.body.style.overflow=""}function l(e,i){if(!n)return;const s=n.querySelector("#"+e);s&&(s.textContent=i,s.hidden=!1)}function u(e){if(!n)return;const i=n.querySelector("#"+e);i&&(i.hidden=!0)}function S(e){e.addEventListener("click",t=>{t.target.closest("[data-sin-close]")&&f(),t.target.closest("[data-sin-back]")&&m(1)}),document.addEventListener("keydown",t=>{t.key==="Escape"&&n&&!n.hidden&&f()});const i=e.querySelector("#sinEmail"),s=e.querySelector("#sinSendBtn"),r=e.querySelector("#sinCode"),o=e.querySelector("#sinVerifyBtn");s.addEventListener("click",async()=>{u("sinEmailError");const t=i.value.trim();if(!t||!t.includes("@")||t.length<5){l("sinEmailError","That doesn't look like an email — try again.");return}const c=Date.now();if(c-_<y){const E=Math.ceil((y-(c-_))/1e3);l("sinEmailError",`Easy — wait ${E}s before trying again.`);return}s.disabled=!0;const p=s.textContent;s.textContent="Sending…";const{error:d}=await h(t);if(s.disabled=!1,s.textContent=p,d){l("sinEmailError","Couldn't send code — "+(d.message||"try again later"));return}_=Date.now();const a=e.querySelector("#sinEmailEcho");a&&(a.textContent=t),r.dataset.email=t,m(2)}),i.addEventListener("keydown",t=>{t.key==="Enter"&&s.click()}),i.addEventListener("input",()=>u("sinEmailError")),o.addEventListener("click",async()=>{u("sinCodeError");const t=r.value.trim(),c=r.dataset.email||"";if(!t||t.length<6){l("sinCodeError","Pop in the 6-digit code from your email.");return}o.disabled=!0;const p=o.textContent;o.textContent="Verifying…";const{error:d}=await b(c,t);if(d){o.disabled=!1,o.textContent=p,l("sinCodeError","That code didn't work — "+(d.message||"try again"));return}m(3);try{await new Promise(a=>setTimeout(a,250)),await g()}catch(a){console.warn("[SignInModal] cloudPull failed:",a)}try{localStorage.setItem("harvest_seen_landing","1")}catch{}window.location.reload()}),r.addEventListener("keydown",t=>{t.key==="Enter"&&o.click()}),r.addEventListener("input",()=>u("sinCodeError"))}function C(){n||(n=v()),n.hidden=!1,m(1)}export{C as showSignInModal};
