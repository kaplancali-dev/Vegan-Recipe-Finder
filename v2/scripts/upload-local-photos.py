#!/usr/bin/env python3
import time
from pathlib import Path
import requests

SUPABASE_URL = 'https://zhncgdbhgkeiybdbzsql.supabase.co'
SUPABASE_BUCKET = 'recipe-images'
SUPABASE_SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpobmNnZGJoZ2tlaXliZGJ6c3FsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NjYyNzcyNywiZXhwIjoyMDkyMjAzNzI3fQ.PW58eQ6vioix4tuRvNKzc4R5kW2flZKL4i3_9bGrfhc'

PHOTO_DIR = Path(__file__).parent / 'photos'
photos = sorted(PHOTO_DIR.glob('*.jpg'))
print(f"Found {len(photos)} photos to upload\n")
uploaded = 0
failed = 0
for i, photo in enumerate(photos, 1):
    data = photo.read_bytes()
    url = f"{SUPABASE_URL}/storage/v1/object/{SUPABASE_BUCKET}/{photo.name}"
    try:
        resp = requests.post(url, headers={'Authorization': f'Bearer {SUPABASE_SERVICE_KEY}', 'Content-Type': 'image/jpeg', 'x-upsert': 'true'}, data=data, timeout=30)
        resp.raise_for_status()
        uploaded += 1
        print(f"[{i}/{len(photos)}] Uploaded {photo.name} ({len(data)/1024:.0f} KB)")
    except Exception as e:
        failed += 1
        print(f"[{i}/{len(photos)}] FAILED {photo.name}: {e}")
    if i % 20 == 0:
        time.sleep(1)
print(f"\nDone! Uploaded: {uploaded}, Failed: {failed}")
